...
const modules={
 ...
 training:{label:'Training',landing:'admin-lms-courses.html',product:'training',groups:[
  ...
  ['Course Management',[
   ['admin-lms-courses.html','Courses'],['admin-lms-creation.html','Create Course'],['admin-lms-lessons.html','Lessons'],['admin-lms-assessments.html','Assessments'],['admin-lms-quizzes.html','Quizzes'],['admin-lms-media.html','Media'],['admin-lms-course-engagement.html','Course Engagement']]]]},
 workforce:{label:'Workforce',landing:'admin-workforce-ctpas.html',product:'workforce',groups:[
  ['Customers & Access',[
   ['admin-workforce-ctpas.html','C/TPA Accounts'],['admin-workforce-employers.html','Non-DOT Employers'],['admin-workforce-employees.html','Employees'],['admin-workforce-onboarding.html','Onboarding'],['admin-workforce-portal-access.html','Portal Access'],['admin-customer-assignments.html','Staff Assignments']]],
  ['Programs & Consortiums',[
   ['admin-workforce-programs.html','Non-DOT Programs'],['admin-workforce-pools.html','Consortiums & Random Pools'],['admin-workforce-pool-members.html','Pool Memberships'],['admin-workforce-selections.html','Random Selections']]],
  ['Operations & Compliance',[
   ['admin-workforce-services.html','Service Configuration'],['admin-workforce-compliance.html','Compliance Cases'],['admin-workforce-policies.html','Policies & Acknowledgments'],['admin-workforce-documents.html','Documents'],['admin-workforce-training.html','Training Records'],['admin-workforce-notifications.html','Notifications'],['admin-workforce-reports.html','Reports']]]]},
 dot:{label:'DOT',landing:'admin-dot-ctpas.html',product:'dot',groups:[
  ...
  ['Programs & Randoms',[
   ['admin-dot-programs.html','DOT Programs'],['admin-dot-consortiums.html','Consortiums'],['admin-dot-pools.html','Random Pools'],['admin-dot-pool-members.html','Pool Memberships'],['admin-dot-selections.html','Random Selections']]],
  ['Regulatory Operations',[
   ['admin-dot-clearinghouse.html','Clearinghouse'],['admin-dot-new-entrant.html','New Entrant Audits'],['admin-dot-rtd.html','Return-to-Duty / SAP'],['admin-dot-compliance.html','Compliance Cases'],['admin-dot-services.html','DOT Service Catalog'],['admin-dot-documents.html','Documents'],['admin-dot-notifications.html','Notifications'],['admin-dot-reports.html','Reports']]]]},
 finance:{label:'Finance',landing:'admin-finance-ar.html',permission:c=>!c||c.super_admin||(c.permissions||[]).some(x=>String(x).startsWith('finance.')),groups:[
  ['Receivables',[
   ['admin-finance-ar.html','Accounts Receivable'],['admin-invoices.html','Invoices'],['admin-invoice.html','Invoice Detail']]],
  ['Payables',[
   ['admin-finance-ap.html','Accounts Payable']]],
  ['Commerce',[
   ['admin-orders.html','Commerce Orders'],['admin-checkout.html','Checkout'],['admin-discounts.html','Discounts']]]]} 
};
const explicit={
 ...
 'admin-dot.html':'dot','admin-dot-ctpas.html':'dot','admin-dot-employers.html':'dot','admin-dot-owner-operators.html':'dot','admin-dot-drivers.html':'dot','admin-dot-portal-access.html':'dot','admin-dot-programs.html':'dot','admin-dot-consortiums.html':'dot','admin-dot-pools.html':'dot','admin-dot-pool-members.html':'dot','admin-dot-selections.html':'dot','admin-dot-clearinghouse.html':'dot','admin-dot-new-entrant.html':'dot','admin-dot-rtd.html':'dot','admin-dot-compliance.html':'dot','admin-dot-services.html':'dot','admin-dot-documents.html':'dot','admin-dot-notifications.html':'dot','admin-dot-reports.html':'dot',
 'admin-finance-ar.html':'finance','admin-finance-ap.html':'finance','admin-invoices.html':'finance','admin-invoice.html':'finance','admin-checkout.html':'finance','admin-discounts.html':'finance'
};
...
function navItem([href,label,icon],activeModule){const active=page===href;const ic=icon||moduleIcons[activeModule]||'home';return `<a class="ep-side-link ${active?'active':''}" href="${href}"><span class="ep-side-icon">${ICONS[ic]||ICONS.home}</span><span>${escapeHtml(label)}</span></a>`}
function sidebar(){const key=moduleForPage(),m=modules[key]||modules.testing;return `<aside class="ep-sidebar"><div class="ep-brand"><a href="${m.landing}"><img src="images/logo2.png" alt="screenings4u"><span>screenings4u Enterprise</span></a></div><nav class="ep-side-scroll" aria-label="${escapeHtml(m.label)} management">${m.groups.map(([title,links])=>`<div class="ep-side-label">${escapeHtml(title)}</div><div class="ep-side-section">${links.map(x=>navItem(x,key)).join('')}</div>`).join('')}<div class="ep-side-divider"></div><div class="ep-side-label">Administration</div><div class="ep-side-section">${adminItems.map(x=>navItem(x,key)).join('')}</div></nav><div class="ep-side-promo"><div class="ep-side-promo-image"></div><div class="ep-side-promo-copy"><strong>Safer People.<br>Stronger Workplaces</strong><span>Compliance today.<br>Opportunities tomorrow.</span><i></i></div></div></aside>`}
function tabDropdown(key,m){return `<div class="ep-biz-tab-wrap"><a class="ep-biz-tab ${moduleForPage()===key?'active':''}" href="${m.landing}">${escapeHtml(m.label)}</a></div>`}
...
function wire(){const overlay=$('#epOverlay');$('#epMobileMenu')?.addEventListener('click',()=>document.body.classList.add('ep-mobile-open'));overlay?.addEventListener('click',()=>{document.body.classList.remove('ep-mobile-open');closeFloating()});$$('.ep-side-link').forEach(a=>a.addEventListener('click',()=>document.body.classList.remove('ep-mobile-open')));
...
async function loadContext(){try{const sb=window.screenings4uSupabase;if(!sb?.functions)return;const {data,error}=await sb.functions.invoke('screenings4u-staff-context');if(!error&&data?.user)state.ctx={...data,super_admin:data.is_super_admin,product_access:data.products||data.product_access||[]}}catch(e){console.warn('[Enterprise Shell] staff context unavailable',e)}}
