...
function fail(e){console.error(e);const page=document.querySelector('.ep-business-page');if(!page)return;let n=document.getElementById('dotAgencyError');if(!n){n=document.createElement('div');n.id='dotAgencyError';n.className='ep-page-notice';page.prepend(n)}n.textContent=e?.message||String(e)}
...
function rowButton(label,action,id){const href=action==='registration'?`admin-dot-agency-record.html?agency=${encodeURIComponent(agency)}&resource=registration&employer_id=${encodeURIComponent(id)}`:`admin-dot-agency-record.html?agency=${encodeURIComponent(agency)}&resource=program&id=${encodeURIComponent(id)}`;return `<a class="ep-business-btn ep-row-link" href="${href}">${esc(label)}</a>`}
...
function workerRows(){return (state.data?.employees||[]).map(x=>`<tr><td><strong>${esc(workerName(x))}</strong><span class="sub">${esc(x.employee_number||'—')}</span></td><td>${esc(empName(x.employer_id))}</td><td>${esc(x.job_title||'—')}</td><td>${esc(x.dot_agency||agency)}</td><td>${badge(x.employment_status)}<div class="ep-record-inline-actions"><a class="ep-business-btn ep-row-link" href="admin-dot-agency-record.html?agency=${encodeURIComponent(agency)}&resource=worker&id=${encodeURIComponent(x.id)}">Manage</a></div></td></tr>`).join('')||'<tr><td colspan="5"><div class="ep-business-empty">No covered workers found.</div></td></tr>'}
...
function testingRows(){return (state.data?.testing_orders||[]).slice(0,250).map(x=>`<tr><td>${esc(x.order_number||x.id)}</td><td>${esc(empName(x.employer_id))}</td><td>${esc(String(x.reason||'—').replaceAll('_',' '))}</td><td>${esc(x.test_type||'—')}</td><td>${badge(x.status)}<div class="ep-record-inline-actions"><a class="ep-business-btn ep-row-link" href="admin-dot-record.html?resource=testing_order&id=${encodeURIComponent(x.id)}">Manage</a></div></td></tr>`).join('')||'<tr><td colspan="5"><div class="ep-business-empty">No testing orders found.</div></td></tr>'}
function accidentRows(){return (state.data?.post_accident_events||[]).slice(0,250).map(x=>{const tid=x.drug_testing_order_id||x.alcohol_testing_order_id;const href=tid?`admin-dot-record.html?resource=testing_order&id=${encodeURIComponent(tid)}`:`admin-dot-customer.html?type=employer&id=${encodeURIComponent(x.employer_id)}#operations`;return `<tr><td>${fmt(x.occurred_at)}</td><td>${esc(empName(x.employer_id))}</td><td>${esc(x.accident_type||'Agency event')}</td><td>${badge(x.testing_required===true?'testing required':x.testing_required===false?'not required':'pending')}</td><td>${badge(x.status)}<div class="ep-record-inline-actions"><a class="ep-business-btn ep-row-link" href="${href}">Manage</a></div></td></tr>`}).join('')||'<tr><td colspan="5"><div class="ep-business-empty">No post-accident events found.</div></td></tr>'}
function complianceRows(){return (state.data?.compliance_cases||[]).slice(0,250).map(x=>`<tr><td>${esc(x.case_number||x.id)}</td><td>${esc(empName(x.employer_id))}</td><td>${esc(x.event_type||'—')}</td><td>${badge(x.priority)}</td><td>${badge(x.status)}<div class="ep-record-inline-actions"><a class="ep-business-btn ep-row-link" href="admin-dot-record.html?resource=compliance&id=${encodeURIComponent(x.id)}">Manage</a></div></td></tr>`).join('')||'<tr><td colspan="5"><div class="ep-business-empty">No compliance cases found.</div></td></tr>'}
function selectionRows(){return (state.data?.selections||[]).slice(0,250).map(x=>`<tr><td>${fmt(x.selection_date)}</td><td>${esc(x.pool_id||'—')}</td><td>${esc(x.population_size||0)}</td><td>${esc(x.drug_selection_count||0)} / ${esc(x.alcohol_selection_count||0)}</td><td>${badge(x.status)}<div class="ep-record-inline-actions"><a class="ep-business-btn ep-row-link" href="admin-dot-record.html?resource=selection&id=${encodeURIComponent(x.id)}">Manage</a></div></td></tr>`).join('')||'<tr><td colspan="5"><div class="ep-business-empty">No random selection events found.</div></td></tr>'}
...
function init(){
 ...
 $('#addRegistration')?.addEventListener('click',()=>{const first=state.data?.employers?.[0];if(!first)return fail(new Error(`No ${agency} employers are available.`));location.href=`admin-dot-agency-record.html?agency=${encodeURIComponent(agency)}&resource=registration&employer_id=${encodeURIComponent(first.id)}`});
 $('#addProgram')?.addEventListener('click',()=>location.href=`admin-dot-agency-record.html?agency=${encodeURIComponent(agency)}&resource=program&mode=new`);
 $('#addWorker')?.addEventListener('click',()=>location.href=`admin-dot-agency-record.html?agency=${encodeURIComponent(agency)}&resource=worker&mode=new`);
 load().catch(fail);
}
